...
bigu_prod_mast_sheet = xlrd.open_workbook(prefix_path + '/Master File WM/WM Product Master in BigU.xlsx').sheet_by_name(
    u'Sheet')
...
def gen_fmot_sub_df(dm_num, start_date, end_date, cust_extrn_id, prod_extrn_id, bigu_gtin, bigu_barcode, bigu_item_desc, upc, barcode, item_desc, dm_question, display_type):
    ...
    for sales_date in daterange(start_date, end_date):
        t_df = t_df.append({'srce_sys_id': srce_sys_id,
                            ...
                            'bigu_gtin': bigu_gtin,
                            'big_barcode': bigu_barcode,
                            'big_item_desc': None,  # bigu_item_desc,  endoc has issue with bigu item desc
                            'upc': upc,
                            'barcode': barcode,
                            'item_desc': item_desc,
                            'dm_num': dm_num,
...
def gen_dm_sku_store_fmot(dm_num, start_date, end_date, bigu_prod_mast_sheet, bigu_mast_store_df, wm_store_mast_sheet):
    ...
    for sfa_code in dm_sheet.col(0)[5:-1]:
        cust_extrn_id = get_cust_extrn_id(sfa_code.value, bigu_mast_store_df, wm_store_mast_sheet)
...
def get_bigu_prod_dt(upc, bigu_prod_mast_sheet):
    ...
    return prod_extrn_id, bigu_gtin, bigu_barcode, bigu_item_desc


def parallel_gen_res(row):
    res_df, wrong_store_df = gen_dm_sku_store_fmot(row['DM'], row['Start Date'].date(), row['End Date'].date(),
                                                   bigu_prod_mast_sheet, bigu_mast_store_df, wm_store_mast_sheet)
...
def main():
    ...
    #     res_df, wrong_store_df = gen_dm_sku_store_fmot(row['DM'], row['Start Date'].date(), row['End Date'].date(), bigu_prod_mast_sheet, bigu_mast_store_df, wm_store_mast_sheet)
    #
    #     print row['DM']
    #     res_df.to_csv(prefix_path + '/Output/DM FMOT/SKU DM %s FMOT DIM.csv' % row['DM'])
    #     wrong_store_df.to_csv(prefix_path + '/Output/DM FMOT/DM %s Wrong Stores.csv' % row['DM'])
    ...
    res = Parallel(n_jobs=num_cores)(delayed(parallel_gen_res)(row) for index, row in dm_date_dict.iterrows())
    print res
    print 'Done'
