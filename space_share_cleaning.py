...
def batch_process_space_share(dir_path):
    ...
    all_data.to_csv("C:/Users/liu.y.25/Desktop/Promo Forecast/2 DM Data/Fem/Display Space/P2Y Display Space for Hive.csv", encoding='gbk')
...
def find_data_missing_store(df, month):
    missing_list = df[(df["Category"] == u"总计") & (df["P&G Size"] == 0) & (df["Cat Size"] == 0)][
        ["Store Name", "SFA Code"]]
    missing_num = missing_list["SFA Code"].count()
    store_num = len(df["SFA Code"].unique())
    t_path = "C:/Users/liu.y.25/Desktop/Promo Forecast/2 DM Data/Fem/Display Space/Time %s Total %d Missing %d.csv" % (month, store_num, missing_num)
    missing_list.to_csv(t_path, encoding='gbk')


def main():
    # strict dir_path
    dir_path = "C:/Users/liu.y.25/Desktop/Promo Forecast/2 DM Data/Fem/Display Space/201*.csv"
    ...
    batch_process_space_share(dir_path)
