...
def compare_dm_scorecard(sc_path):
    for path in glob.glob(sc_path):
        df = pd.read_csv(path, encoding='gbk')
        dm_date = get_dm_date(path)
        ...
        for col in df.columns.values:
            for item, upc in zip(dict_df[u'Item '], dict_df[u'Upc']):
                if any(s in col for s in ignore_cols):
                    continue
                ...
                if l_ratio > 0.5:
                    ...
                    unmatched.append([col, item, upc, l_ratio, dm_date[0], dm_date[1]])
                    unmatched_cnt += 1

        # Save match and unmatched results
